...
export default {
  ...
  computed: {
    ...
    resSaveUrl () {
      if (this.resPropsData.type === 'resReviseRest') {
        return '/approve/updateArbitrator'
      } else if (this.resPropsData.type === 'resRevise') {
        return '/approve/updateGroupApproveToArbitrator'
      } else if (this.resPropsData.type === 'resReviseEdit' || this.resPropsData.type === 'resReviseEditBatch') {
        return '/approve/confirmArbitrator'
      } else if (this.resPropsData.type === 'resReviseBatch') {
        return '/batchGroupCourt/updateChooseArbitratorBatch'
      } else {
        return ''
      }
    }
  ...
  methods: {
    ...
    alertSave () {
      ...
      if (['resReviseRest', 'resRevise', 'resReviseEdit', 'resReviseEditBatch'].indexOf(this.resPropsData.type) !== -1) {
        _data.caseId = this.resPropsData.caseId
        _data.arbitratorIds = this.seleArr.join(',')
      }
      if (this.resPropsData.type === 'resRevise') {
        _data.tribunalRequestId = this.resPropsData.resTribId
      }
      if (this.resPropsData.type === 'resReviseBatch') {
        _data.caseTribunalRequest = this.resPropsData.caseTribunalRequest
        _data.arbitratorIds = this.seleArr.join(',')
      }
      axios.post(this.resSaveUrl, _data).then(res => {
